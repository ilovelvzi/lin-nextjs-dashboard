...
const CreateResumeSchema = z.object({
  ...
  originalContent: z.string().min(10, { message: '简历内容太短，请输入更多内容。' }),
...
export async function createResume(
  ...
  const { title, originalContent, jobDescription } = validated.data;

  try {
    const rows = await sql<{ id: string }[]>`
      INSERT INTO resumes (user_id, title, original_content, job_description, status)
      VALUES (
        ${session.user.id},
        ${title},
        ${originalContent},
